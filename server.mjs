  ...
  async function initiateSession() {
      ...
      if (connection === "open") {
        ...
        try {
          ...
          if (!result.ok) {
            ...
            // CRITICAL: do NOT call sock.logout() — that sends <remove-companion-device>
            // and unlinks the freshly-paired device, which makes the SESSION_ID die
            // 1-2s after the bot tries to use it. Just close the WS instead.
          ...
          // ─── CRITICAL FIX (v2.0.2) ─────────────────────────────────
          // Previously this called `sock.logout()` which sends an IQ
          // telling WhatsApp to REMOVE the freshly-paired companion
          // device. The bot would then connect with a SESSION_ID for
          // a device that no longer exists in the user's linked-
          // devices list, and WhatsApp would disconnect it as
          // `loggedOut` within 1-2 seconds. The fix is to drop the
          // WebSocket WITHOUT sending the logout IQ — the device
          // stays linked and the SESSION_ID stays valid.
          await delay(1500);
          try { sock.ws?.close?.(); } catch {}
          try { sock.end?.(undefined); } catch {}
        } catch (err) {
    ...
    if (!sock.authState.creds.registered) {
      ...
      try {
        ...
        // Don't call sock.logout() — pairing failed before the device was even linked,
        // and even if it weren't, logout() would delete the link. Just close the WS.
        try { sock.ws?.close?.(); } catch {}
        try { sock.end?.(undefined); } catch {}
        removeFile(dirs);
      }
    ...
    const handleUpdate = async (update) => {
      ...
      if (connection === "open") {
        ...
        try {
          ...
          if (!result.ok) {
            log("❌ Session not ready:", result.reason);
            // CRITICAL: do NOT call sock.logout() — see pair-code flow above.
            try { sock.ws?.close?.(); } catch {}
            try { sock.end?.(undefined); } catch {}
            return;
          }
          ...
          log("📤 SESSION_ID delivered to +" + result.phone);
          // ─── CRITICAL FIX (v2.0.2) — see pair-code flow note ───
          // sock.logout() unlinks the device; closing the WS leaves it linked.
          await delay(1500);
          try { sock.ws?.close?.(); } catch {}
          try { sock.end?.(undefined); } catch {}
        } catch (e) {
